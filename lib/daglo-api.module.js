  ...
  stream = {
    transcriber: () => {
      let lib = {
        ...
        baseFunction: async (stream, sttConfig = undefined, enableCallback = false) => {
            ...
            if (!lib.model.audioCtx) {
              ...
              scriptNode.onaudioprocess = (audioProcessingEvent) => {
                ...
                //Debug callback -only if enabled
                if (enableCallback && typeof(lib.utils.__callback['debug']) == 'function') {
              ...
              //Stop callback -only if enabled
              if (enableCallback && typeof(lib.utils.__callback['stop']) == 'function') {
         ...
         * This function requests microphone access, initializes the audio processing pipeline,
         * and begins streaming audio data for speech recognition.
         *
         * #param {Object} options - Configuration options for speech recognition.
         * #param {number} [options.threshold=DagloAPI.DEFAULT_CONFIG.threshold] - Minimum confidence score for speech detection.
         * #param {number} [options.concatLevel=DagloAPI.DEFAULT_CONFIG.concatLevel] - Number of consecutive frames required to detect speech.
         * #param {Object} [options.sttConfig] - Additional configuration for speech-to-text (STT) processing.
         * #returns {Promise<void>} - Resolves when the streaming process starts successfully.
         */
        start: async ({threshold = DagloAPI.DEFAULT_CONFIG.threshold, concatLevel = DagloAPI.DEFAULT_CONFIG.concatLevel, sttConfig = undefined} = {}) => {
          ...
          // Validate and process sttConfig
          ...
          // Call baseFunction with the acquired stream, sttConfig, and enable callbacks
          await lib.baseFunction(stream, sttConfig, true);

          // Call start callback after successful initialization
          if (typeof(lib.utils.__callback['start']) == 'function') {
            lib.utils.__callback['start']();
          }
        },
        /**
         * Connects an external audio stream for speech recognition.
         * This function allows using an existing MediaStream instead of requesting a new one.
         *
         * #param {MediaStream} stream - The audio stream to process.
         * #returns {Promise<void>} - Resolves when the stream is successfully connected.
         */
        connect: async (stream) => {
          await lib.baseFunction(stream);
        }
      };
